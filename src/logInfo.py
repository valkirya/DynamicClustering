...
class AppLogging ():
    ...
    def metricsMessage (min_num_cluster, num_tests, num_cluster):
        logging.info('Initial metrics generated, minimum number of cluster {}: , number of tests: {}, initial number of cluster {}'.format(str(min_num_cluster), str(num_tests) , str(num_cluster)) )
    ...
    def dataMessage (key, value):
        logging.info('Running filtered data {}: , Data size: {}'.format(str(key), str(len(value))) )
    ...
    def aglomerativeMessage (clustersNames):    
        logging.info('Call Aglomerative algorithm for clusters of sizes: {}'.format(clustersNames))
